...
for i in range(start,end+1):
    ...
    h_G1=float(data_dict["h_G1"][i])
    h_G2=float(data_dict["h_G2"][i])
    ...
    layers_deck_1_6 = ['elset_plate_1','elset_plate_2','elset_plate_3','elset_plate_4','elset_plate_5','elset_plate_6']
    layers_deck_7_9 = ['elset_plate_7','elset_plate_8','elset_plate_9']
    
    group_str="elset_wall_1"
    layers_wall1 = [l for l in layer_names if group_str in l]
    layers_wall1_1_3=['elset_wall_1_1','elset_wall_1_2','elset_wall_1_3']
    layers_wall1_4_6= ['elset_wall_1_4','elset_wall_1_5','elset_wall_1_6']
    layers_wall1_7_9= ['elset_wall_1_7','elset_wall_1_8','elset_wall_1_9']
    group_str="elset_wall_2"
    layers_wall2= [l for l in layer_names if group_str in l]
    layers_wall2_1_3=['elset_wall_2_1','elset_wall_2_2','elset_wall_2_3']
    layers_wall2_4_6= ['elset_wall_2_4','elset_wall_2_5','elset_wall_2_6']
    layers_wall2_7_9= ['elset_wall_2_7','elset_wall_2_8','elset_wall_2_9']
    ...
    d4 = float(data_dict["d4_plate_1_6"][i])
    ...
    name_mat='plate_element_mat_1_6'
    ...
    d2 = float(data_dict["d2_plate"][i]) 
    d3 = float(data_dict["d3_plate"][i])
    d4 = float(data_dict["d4_plate_7_9"][i])
    ...
    reinf_3L={'stahl':1,'zm':2,'fsy':fsy,'fsu':fsu,'esu':esu,'esv':0.02,'Es':200000,'ka':-1,'kb':-1,'kc':-1,'as':as3,'dm':d3,'psi':0}
    reinf_4L={'stahl':1,'zm':2,'fsy':fsy,'fsu':fsu,'esu':esu,'esv':0.02,'Es':200000,'ka':-1,'kb':-1,'kc':-1,'as':as4,'dm':d4,'psi':90}
    name_mat='plate_element_mat_7_9'
    mdl.add(CMMUsermat(name=name_mat, geo=geo, concrete=concrete, reinf_1L=reinf_1L, reinf_2L=reinf_2L, reinf_3L=reinf_3L, reinf_4L=reinf_4L,))
    ...
    dmin=min(d1,d2,d3,d4)
    nn_min=m.ceil(tmin/dmin) #auf runden
    name_sec ='plate_element_sec_7_9'
    mdl.add(ShellSection(name=name_sec, t=t_p, semi_loc_coords=semi_loc_coords, nn=nn_min, offset_mode='top'))
    
    
    
    for plate in layers_deck_7_9:       
        # -- define elset properties -- 
        name_prop = '{}_element_prop'.format(plate)
        mdl.add(Properties(name=name_prop, material=name_mat, section=name_sec, elset=plate))
    ...
    #--read out sampled local parameters from the read in dict--
    # reinforcement geometry                
    d1 = float(data_dict["d1_walls_1_6"][i]) #[mm]
    d2 = float(data_dict["d2_walls"][i]) 
    d3 = float(data_dict["d3_walls"][i])
    d4 = float(data_dict["d4_walls_1_3"][i])
    ...
    name_sec ='wall2_element_sec_1_3'
    mdl.add(ShellSection(name=name_sec, t=t_w, semi_loc_coords=semi_loc_coords, nn=nn_min, offset_mode='mid'))
    
    
    for wall in layers_wall2_1_3:
    ...
    d2 = float(data_dict["d2_walls"][i]) 
    d3 = float(data_dict["d3_walls"][i])
    d4 = float(data_dict["d4_walls_4_6"][i])
    ...
    # -- define section-- 
    semi_loc_coords=calc_loc_coor(layer=layers_wall1[0], PORIG=[0,0,-h_w_m],PXAXS=[1,0,0]) 
    name_sec ='wall1_element_sec_4_6'
    mdl.add(ShellSection(name=name_sec, t=t_w, semi_loc_coords=semi_loc_coords, nn=nn_min,offset_mode='mid' ))
    
    for wall in layers_wall1_4_6:
        # -- define elset properties -- 
        name_prop = '{}_element_prop'.format(wall)
        mdl.add(Properties(name=name_prop, material=name_mat, section=name_sec, elset=wall))
    ...
    # -- define section-- 
    semi_loc_coords=calc_loc_coor(layer=layers_wall2[0], PORIG=[0,L_el,-h_w_m],PXAXS=[1,0,0])     
    name_sec ='wall2_element_sec_4_6'
    mdl.add(ShellSection(name=name_sec, t=t_w, semi_loc_coords=semi_loc_coords, nn=nn_min, offset_mode='mid'))
    
    
    for wall in layers_wall2_4_6:
        #--define section and property--
        name_prop = '{}_element_prop'.format(wall)
        mdl.add(Properties(name=name_prop, material=name_mat, section=name_sec, elset=wall))
    ...
    #--read out sampled local parameters from the read in dict--
    # reinforcement geometry                
    d1 = float(data_dict["d1_walls_7_9"][i]) #[mm]
    print(d1)
    d2 = float(data_dict["d2_walls"][i]) 
    d3 = float(data_dict["d3_walls"][i])
    d4 = float(data_dict["d4_walls_7_9"][i])
    print(d4)
    ...
    # -- define section-- 
    semi_loc_coords=calc_loc_coor(layer=layers_wall1[0], PORIG=[0,0,-h_w_m],PXAXS=[1,0,0]) 
    name_sec ='wall1_element_sec_7_9'
    mdl.add(ShellSection(name=name_sec, t=t_w, semi_loc_coords=semi_loc_coords, nn=nn_min,offset_mode='mid' ))
    
    for wall in layers_wall1_7_9:
        # -- define elset properties -- 
        name_prop = '{}_element_prop'.format(wall)
        mdl.add(Properties(name=name_prop, material=name_mat, section=name_sec, elset=wall))
    ...
    # -- define section-- 
    semi_loc_coords=calc_loc_coor(layer=layers_wall2[0], PORIG=[0,L_el,-h_w_m],PXAXS=[1,0,0])     
    name_sec ='wall2_element_sec_7_9'
    mdl.add(ShellSection(name=name_sec, t=t_w, semi_loc_coords=semi_loc_coords, nn=nn_min, offset_mode='mid'))
    
    
    for wall in layers_wall2_7_9:
        #--define section and property--
        name_prop = '{}_element_prop'.format(wall)
        mdl.add(Properties(name=name_prop, material=name_mat, section=name_sec, elset=wall))
...
#    plot_nr_nodes(mdl)  # Plot Node Numbers
    ...
    earth_pressure_gravel_load=earthPressure_gravel_generator(structure=mdl, elements=layers_deck, h_G=h_G1, gamma_E=gamma_E, phi_k=phi_k, gamma_G=1.35)
    ...
    earth_pressure_backfill_load = earthPressure_backfill_generator(structure=mdl, elements=layers_walls, h_w=h_w, t_p=t_p, 
                                                                    h_G=h_G1, gamma_E=gamma_E, phi_k=phi_k, gamma_G=1.35 )  #[N/mm2]

    ## Live Loads
    #Normalspurverkehr Load generator 
    y_A_Biegung=(L_el/2) #-(m.cos(m.radians(beta))*1500)
    NSV_load_names1=Normalspurbahnverkehr_load_generator(mdl,name='Gleis1', l_Pl=L_el, h_Pl=t_p, s=s1*b1, beta=beta1,
                                                         q_Gl=q_Gl, b_Bs=b_Bs, h_Strich=h_G2,h_GL=160, h_w=h_w, Q_k=Q_k, y_A=y_A_Biegung,m=4650,
    ...
    NSV_load_names2=Normalspurbahnverkehr_load_generator(mdl,name='Gleis2', l_Pl=L_el, h_Pl=t_p, s=s2*b1, beta=beta2,
                                                         q_Gl=q_Gl, b_Bs=b_Bs, h_Strich=h_G2,h_GL=160, h_w=h_w, Q_k=Q_k, y_A=y_A_Biegung,m=4650,
                                                         gamma_G=1.35,direction='all', gamma_Q=1.45, verbalise=True)
    ...
    earth_pressure_liveload1 = earthPressure_liveload_generator(structure=mdl, s=s1*b1,beta=beta1, L=L_el, h_w=h_w, t_p=t_p, 
                                                                phi_k=phi_k, direction='all', gamma_Q=1.45, name='Gleis1')
                                                                
    earth_pressure_liveload2 = earthPressure_liveload_generator(structure=mdl, s=s2*b1,beta=beta2, L=L_el, h_w=h_w, t_p=t_p, 
                                                                phi_k=phi_k, direction='all', gamma_Q=1.45, name='Gleis2')
    ...
    live_loads = NSV_live_loads1 +earth_pressure_liveload1 + NSV_live_loads2 +earth_pressure_liveload2
