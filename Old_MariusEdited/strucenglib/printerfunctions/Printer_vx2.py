...
def cleaner(plotcc, plot):
    ...
    for element in plotcc:
        ...
        if (k >= 0 and len(plotcc) > k + 1) and (
                (plotcc[k] == None and plotcc[k - 1] != None and plotcc[k + 1] == None) or (
                plotcc[k] == None and plotcc[k - 1] == None and plotcc[k + 1] != None)):
            plotcc_neu[k] = plot[k]
        else:
...
res_dic = {
    "as_xi_bot": {"aaa": [1], "bbb": [0], "xmax": 701, "ymax": 1000, "ylabelname": "$a_{s,x,bot}$ [mm\u00b2/m]",
                  "ylabelname2": "$a_{s,\u03BE,bot}$ [mm\u00b2/m]"},
    "as_xi_top": {"aaa": [1], "bbb": [1], "xmax": 701, "ymax": 1000, "ylabelname": "$a_{s,x,top}$ [mm\u00b2/m]",
                  "ylabelname2": "$a_{s,\u03BE,top}$ [mm\u00b2/m]"},
    "as_eta_bot": {"aaa": [2], "bbb": [0], "xmax": 701, "ymax": 1000, "ylabelname": "$a_{s,y,bot}$ [mm\u00b2/m]",
                   "ylabelname2": "$a_{s,\u03B7,bot}$ [mm\u00b2/m]"},
    "as_eta_top": {"aaa": [2], "bbb": [1], "xmax": 701, "ymax": 1000, "ylabelname": "$a_{s,y,top}$ [mm\u00b2/m]",
                   "ylabelname2": "$a_{s,\u03B7,top}$ [mm\u00b2/m]"},
...
for variabel in variabeln:
    ...
    if variabel == "spezial":
        ...
        legende = ["$n_x$ = -2000 kN/m", "$n_x$ = -1000 kN/m", "$n_x$ =  1000 kN/m", "$n_x$ =  2000 kN/m",
                   "$n_x$ =  0 kN/m", ]  # nx

    elif variabel == "ny":
        ny = [0, -1000, -500, 500, 1000]
        legende = ["$n_y$ = -1000 kN/m", "$n_y$ = -500 kN/m", "$n_y$ =  500 kN/m", "$n_y$ =  1000 kN/m",
                   "$n_y$ =  0 kN/m", ]  # ny

    elif variabel == "nxy":
        nxy = [0, -600, -300, 300, 600]
        legende = ["$n_{xy }$ = -600 kN/m", "$n_{xy }$ = -300 kN/m", "$n_{xy }$ =  300 kN/m", "$n_{xy }$ =  600 kN/m",
                   "$n_{xy }$ =  0 kN/m", ]  # ny

    elif variabel == "vx":
        vx = [0, -2000, -1000, 1000, 2000]
        legende = ["$v_x$ = -2000 kN/m", "$v_x$ = -1000 kN/m", "$v_x$ =  1000 kN/m", "$v_x$ =  2000 kN/m",
                   "$v_x$ =  0 kN/m", ]  # vx

    elif variabel == "vy":
        vy = [0, 10, 30, 60, 100]
        legende = ["$v_y$ = 10 kN/m", "$v_y$ = 30 kN/m", "$v_y$ =  60 kN/m", "$v_y$ =  100 kN/m",
                   "$v_y$ =  0 kN/m", ]  # vy

    elif variabel == "mx":
        mx = [0, -100 * 1000, -50 * 1000, 50 * 1000, 100 * 1000]
        legende = ["$m_{x }$ = -100 kNm/m", "$m_{x }$ = -50 kNm/m", "$m_{x }$ =  50 kNm/m", "$m_{x }$ =  100 kNm/m",
                   "$m_{x }$ =  0 kNm/m", ]

    elif variabel == "my":
        my = [0, -100 * 1000, -50 * 1000, 50 * 1000, 100 * 1000]
        legende = ["$m_{y }$ = -100 kNm/m", "$m_{y }$ = -50 kNm/m", "$m_{y }$ =  50 kNm/m", "$m_{y }$ =  100 kNm/m",
                   "$m_{y }$ =  0 kNm/m", ]

    elif variabel == "mxy":
        mxy = [0, -50 * 1000, -25 * 1000, 25 * 1000, 50 * 1000]
        legende = ["$m_{xy }$ = -50 kNm/m", "$m_{xy }$ = -25 kNm/m", "$m_{xy }$ =  25 kNm/m", "$m_{xy }$ =  50 kNm/m",
                   "$m_{xy }$ =  0 kNm/m", ]
        ...
        legende = ["$\u03B8_{Kern }$ = 25°", "$\u03B8_{Kern }$ = 30°", "$\u03B8_{Kern }$ = 35°",
                   "$\u03B8_{Kern }$ = 40°", "$\u03B8_{Kern }$ = 45°", ]
    ...
    setname = ["_ffv", "_ffs", ]
    settext = ["Mindestbewehrung: Aus - Druckzoneniteration: Aus - Schubnachweis: " + '"' + schubnachweis + '"',
               "Mindestbewehrung: Aus - Druckzoneniteration: Aus - Schubnachweis: 'sia'",
               "Mindestbewehrung: Ein - Druckzoneniteration: Aus - Schubnachweis: " + '"' + schubnachweis + '"']
    j = 0
    for se in sets:
        print('\t' + setname[j])
        ...
        for resultat in resultate:
            ...
            for xx in x:
                ...
                zahl = 0
                inp1 = [1, mx[zahl], my[zahl], mxy[zahl], (xx), vy[zahl], (((xx) ** 2.0 + vy[zahl] ** 2.0) ** 0.5),
                        nx[zahl], ny[zahl], nxy[zahl], h[zahl], 40, 40, fck[zahl], theta[zahl], 435, alpha[zahl],
                        alpha[zahl], beta[zahl], beta[zahl], se[0], se[1], se[2], code, [1, 0, 0], [1, 0, 0], [0, 1, 0],
                        [0, 0, 1]]
                ...
                zahl += 1
                inp2 = [2, mx[zahl], my[zahl], mxy[zahl], (xx), vy[zahl], (((xx) ** 2.0 + vy[zahl] ** 2.0) ** 0.5),
                        nx[zahl], ny[zahl], nxy[zahl], h[zahl], 40, 40, fck[zahl], theta[zahl], 435, alpha[zahl],
                        alpha[zahl], beta[zahl], beta[zahl], se[0], se[1], se[2], code, [1, 0, 0], [1, 0, 0], [0, 1, 0],
                        [0, 0, 1]]
                zahl += 1
                inp3 = [3, mx[zahl], my[zahl], mxy[zahl], (xx), vy[zahl], (((xx) ** 2.0 + vy[zahl] ** 2.0) ** 0.5),
                        nx[zahl], ny[zahl], nxy[zahl], h[zahl], 40, 40, fck[zahl], theta[zahl], 435, alpha[zahl],
                        alpha[zahl], beta[zahl], beta[zahl], se[0], se[1], se[2], code, [1, 0, 0], [1, 0, 0], [0, 1, 0],
                        [0, 0, 1]]
                ...
                if variabel == "spezial":
                    se = [False, False, "sia"]
                zahl += 1
                inp4 = [4, mx[zahl], my[zahl], mxy[zahl], (xx), vy[zahl], (((xx) ** 2.0 + vy[zahl] ** 2.0) ** 0.5),
                        nx[zahl], ny[zahl], nxy[zahl], h[zahl], 40, 40, fck[zahl], theta[zahl], 435, alpha[zahl],
                        alpha[zahl], beta[zahl], beta[zahl], se[0], se[1], se[2], code, [1, 0, 0], [1, 0, 0], [0, 1, 0],
                        [0, 0, 1]]
                zahl += 1
                inp5 = [5, mx[zahl], my[zahl], mxy[zahl], (xx), vy[zahl], (((xx) ** 2.0 + vy[zahl] ** 2.0) ** 0.5),
                        nx[zahl], ny[zahl], nxy[zahl], h[zahl], 40, 40, fck[zahl], theta[zahl], 435, alpha[zahl],
                        alpha[zahl], beta[zahl], beta[zahl], se[0], se[1], se[2], code, [1, 0, 0], [1, 0, 0], [0, 1, 0],
                        [0, 0, 1]]
            ...
            if (variabel == "alpha" or variabel == "beta") and (
                    resultat == "as_xi_bot" or resultat == "as_xi_top" or resultat == "as_eta_bot" or resultat == "as_eta_top"):
