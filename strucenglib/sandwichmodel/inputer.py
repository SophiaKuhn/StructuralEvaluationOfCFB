...
def inputer(structure, data, k, step, Mindestbewehrung, Druckzoneniteration, Schubnachweis, code):
    ...
    mx = statistics.mean(list(sm1)) # Bildet den durchschnitt aller Integrationspunkte des Elements

    #sm2-->my
    sm2 = structure.results[step]['element']['sm2'][k].values()
    my = statistics.mean(list(sm2)) # Bildet den durchschnitt aller Integrationspunkte des Elements
    ...
    nx = statistics.mean(list(sf1)) # Bildet den durchschnitt aller Integrationspunkte des Elements

    #sf2-->ny
    sf2 = structure.results[step]['element']['sf2'][k].values()
    ny = statistics.mean(list(sf2)) # Bildet den durchschnitt aller Integrationspunkte des Elements
    ...
    # Daten aus den Zusaetlichen Parametern (Additionalproperty) (d_strich_bot,d_strich_top,fc_k,theta_grad_kern,fs_d, alpha_bot, alpha_top, beta_bot, beta_top, ex, ey, ez)
    ...
    ex = data[prop_name]['ex']
    ey = data[prop_name]['ey']
    ez = data[prop_name]['ez']
