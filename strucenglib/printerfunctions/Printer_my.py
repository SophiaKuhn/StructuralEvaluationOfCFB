...
res_dic = {
    "as_xi_bot": {"aaa": [1], "bbb": [0], "xmax": 701, "ymax": 10000, "ylabelname": "$a_{s,x,bot}$ [mm\u00b2/m]",
                  "ylabelname2": "$a_{s,\u03BE,bot}$ [mm\u00b2/m]"},
    "as_xi_top": {"aaa": [1], "bbb": [1], "xmax": 701, "ymax": 10000, "ylabelname": "$a_{s,x,top}$ [mm\u00b2/m]",
                  "ylabelname2": "$a_{s,\u03BE,top}$ [mm\u00b2/m]"},
    "as_eta_bot": {"aaa": [2], "bbb": [0], "xmax": 701, "ymax": 10000, "ylabelname": "$a_{s,y,bot}$ [mm\u00b2/m]",
                   "ylabelname2": "$a_{s,\u03B7,bot}$ [mm\u00b2/m]"},
    "as_eta_top": {"aaa": [2], "bbb": [1], "xmax": 701, "ymax": 10000, "ylabelname": "$a_{s,y,top}$ [mm\u00b2/m]",
...
for variabel in variabeln:
    ...
    if variabel == "spezial":
        ...
        legende = ["$v_y$ = -2000 kN/m", "$v_y$ = -1000 kN/m", "$v_y$ =  1000 kN/m", "$v_y$ =  2000 kN/m",
                   "$v_y$ =  0 kN/m", ]  # vy
    ...
    settext = ["Mindestbewehrung: Aus - Druckzoneniteration: Aus - Schubnachweis: " + '"' + schubnachweis + '"',
               "Mindestbewehrung: Aus - Druckzoneniteration: Ein - Schubnachweis: " + '"' + schubnachweis + '"',
    ...
    for se in sets:
        ...
        for resultat in resultate:
            ...
            for xx in x:
                ...
                zahl = 0
                inp1 = [1, mx[zahl], my[zahl], mxy[zahl], vx[zahl], vy[zahl],
                        ((vx[zahl] ** 2.0 + vy[zahl] ** 2.0) ** 0.5), (xx * 3), ny[zahl], nxy[zahl], h[zahl], 40, 40,
                        fck[zahl], theta[zahl], 435, alpha[zahl], alpha[zahl], beta[zahl], beta[zahl], se[0], se[1],
                        se[2], code, [1, 0, 0], [1, 0, 0], [0, 1, 0], [0, 0, 1]]

                if variabel == "spezial":
                    se = [True, False, schubnachweis]

                zahl += 1
                inp2 = [2, mx[zahl], my[zahl], mxy[zahl], vx[zahl], vy[zahl],
                        ((vx[zahl] ** 2.0 + vy[zahl] ** 2.0) ** 0.5), (xx * 3), ny[zahl], nxy[zahl], h[zahl], 40, 40,
                        fck[zahl], theta[zahl], 435, alpha[zahl], alpha[zahl], beta[zahl], beta[zahl], se[0], se[1],
                        se[2], code, [1, 0, 0], [1, 0, 0], [0, 1, 0], [0, 0, 1]]
                zahl += 1
                inp3 = [3, mx[zahl], my[zahl], mxy[zahl], vx[zahl], vy[zahl],
                        ((vx[zahl] ** 2.0 + vy[zahl] ** 2.0) ** 0.5), (xx * 3), ny[zahl], nxy[zahl], h[zahl], 40, 40,
                        fck[zahl], theta[zahl], 435, alpha[zahl], alpha[zahl], beta[zahl], beta[zahl], se[0], se[1],
                        se[2], code, [1, 0, 0], [1, 0, 0], [0, 1, 0], [0, 0, 1]]
                ...
                zahl += 1
                inp4 = [4, mx[zahl], my[zahl], mxy[zahl], vx[zahl], vy[zahl],
                        ((vx[zahl] ** 2.0 + vy[zahl] ** 2.0) ** 0.5), (xx * 3), ny[zahl], nxy[zahl], h[zahl], 40, 40,
                        fck[zahl], theta[zahl], 435, alpha[zahl], alpha[zahl], beta[zahl], beta[zahl], se[0], se[1],
                        se[2], code, [1, 0, 0], [1, 0, 0], [0, 1, 0], [0, 0, 1]]
                zahl += 1
                inp5 = [5, mx[zahl], my[zahl], mxy[zahl], vx[zahl], vy[zahl],
                        ((vx[zahl] ** 2.0 + vy[zahl] ** 2.0) ** 0.5), (xx * 3), ny[zahl], nxy[zahl], h[zahl], 40, 40,
                        fck[zahl], theta[zahl], 435, alpha[zahl], alpha[zahl], beta[zahl], beta[zahl], se[0], se[1],
                        se[2], code, [1, 0, 0], [1, 0, 0], [0, 1, 0], [0, 0, 1]]
